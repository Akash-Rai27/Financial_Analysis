...
AS
SELECT e.City,e.Year,e.[R_D_Expenditure_of_GDP],g.[GDP_in_billion] FROM [dbo].[Economy_Productivity_SD_India] e JOIN 
[dbo].[GDP_Sector_Income_2019_2023_30Cities] g ON e.[City]=g.[City] AND e.Year=g.Year WHERE e.Year=@Search_Year 
AND e.City=@City_name ORDER BY e.City,e.Year;

EXEC R_D_Expense @Search_Year=2019, @City_Name='Ahmedabad';

SELECT e.Year, e.City,([R_D_Expenditure_of_GDP]/[GDP_in_billion])*100 AS R_D_Ratio, 
SUM([Patents_per_100_000_Inhabitants]) AS Patents_per_100_000_Inhabitants FROM [dbo].[Economy_Productivity_SD_India] e JOIN
[dbo].[GDP_Sector_Income_2019_2023_30Cities] g ON e.Year=g.Year AND e.City=g.City 
GROUP BY e.Year,e.City,([R_D_Expenditure_of_GDP]/[GDP_in_billion])*100 ORDER BY Patents_per_100_000_Inhabitants DESC;
...
SELECT City,Year,[GDP_in_billion], LAG([GDP_in_billion]) OVER w AS previous_year_GDP, 
LEAD([GDP_in_billion]) OVER w AS next_year_GDP FROM [dbo].[GDP_Sector_Income_2019_2023_30Cities] 
WINDOW w AS (PARTITION BY City ORDER BY Year);
